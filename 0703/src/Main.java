...
public class Main {
    ...
    private static String convertIp(String ip10) {
        ...
        //Long.parseLong 将字符串参数解析为带符号的十进制 long
        //Long.toBinaryString 返回 long参数的字符串表示形式为基数2中的无符号整数
        String ip2 = Long.toBinaryString(Long.parseLong(ip10));
        String as = "";

        if (ip2.length() < 32){//不足32位，前面补0，补足32位
        ...
        for (int i = 0; i < ips.length; i++) {
            //Integer.parseInt 将字符串参数解析为第二个参数指定的基数中的有符号整数。
    ...
    private static String binaryString(String s) {
        ...
        temp=new StringBuffer(Integer.toBinaryString(Integer.parseInt(s)));
        while(temp.length()<8){
            temp.insert(0, '0');
        }
